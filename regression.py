...
print 'EVALUATION ONE-STEP AHEAD'
...
print '\nEVALUATION K-STEP AHEAD'
mae = ev.calculateMAE(y[sample_num:], prediction_rw)
rmse = ev.calculateRMSE(y[sample_num:], prediction_rw)
print 'RANDOM WALK - MAE:', mae, '- RMSE:', rmse
for t in range (0, 4):
    mae = ev.calculateMAE(y[sample_num:], prediction_error[t])
    rmse = ev.calculateRMSE(y[sample_num:], prediction_error[t])
    print ut.getFunction(t), '- MAE:', mae, '- RMSE:', rmse

labels = ['real values', 'random walk', 'linear', 'polynomial', 'gaussian', 'sigmoid']
ut.plot(labels, y[sample_num:], regression_randomwalk, regression_prediction[0], regression_prediction[1], regression_prediction[2], regression_prediction[3], prediction_rw, prediction_error[0], prediction_error[1], prediction_error[2], prediction_error[3])
